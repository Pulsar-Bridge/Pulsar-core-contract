...
#[test]
fn test_register_transaction_before_initialize_fails() {
    let env = Env::default();
    env.mock_all_auths();
    let contract_id = env.register(PulsarCoreContract, ());
    let client = PulsarCoreContractClient::new(&env, &contract_id);

    let res = client.try_register_transaction(
        &String::from_str(&env, "tx-1"),
        &String::from_str(&env, "GABC123SENDERADDR"),
...
#[test]
fn test_get_relay_signer_before_initialize_fails() {
    let env = Env::default();
    let contract_id = env.register(PulsarCoreContract, ());
    let client = PulsarCoreContractClient::new(&env, &contract_id);

    let res = client.try_get_relay_signer();
    assert_eq!(res, Err(Ok(Error::NotInitialized)));
}
...
#[test]
fn test_register_transaction_rejects_oversized_source_chain() {
    let h = setup();
    let id = tx_id(&h.env, "tx-oversized-source-chain");
    let sender = String::from_str(&h.env, "GABC123SENDERADDR");
    let recipient = Address::generate(&h.env);
    let source_chain = oversized_string(&h.env);
    let dest_chain = String::from_str(&h.env, "stellar");
    let res = h.client.try_register_transaction(
        &id,
        &sender,
...
#[test]
fn test_register_transaction_rejects_empty_dest_chain() {
    let h = setup();
    let id = tx_id(&h.env, "tx-empty-dest-chain");
    let sender = String::from_str(&h.env, "GABC123SENDERADDR");
    let recipient = Address::generate(&h.env);
    let source_chain = String::from_str(&h.env, "ethereum");
    let dest_chain = String::from_str(&h.env, "");
    let res = h.client.try_register_transaction(
        &id,
        &sender,
        &recipient,
...
#[test]
fn test_register_transaction_rejects_oversized_dest_chain() {
    let h = setup();
    let id = tx_id(&h.env, "tx-oversized-dest-chain");
    let sender = String::from_str(&h.env, "GABC123SENDERADDR");
    let recipient = Address::generate(&h.env);
    let source_chain = String::from_str(&h.env, "ethereum");
    let dest_chain = oversized_string(&h.env);
    let res = h.client.try_register_transaction(
        &id,
        &sender,
        &recipient,
...
#[test]
fn test_register_callback_emits_tx_comp_event() {
    use soroban_sdk::{testutils::Events as _, Event as _};
    ...
    let expected = crate::events::TransactionCompleted {
        transaction_id: id,
        updated_at,
    };
    assert_eq!(
...
#[test]
fn test_refunded_transaction_rejects_confirm() {
    let h = setup();
    let id = register_default(&h);
    h.client.refund_transaction(&id);
...
#[test]
fn test_failed_transaction_rejects_callback_completion() {
    let h = setup();
    let id = register_default(&h);
    h.client
        .fail_transaction(&id, &String::from_str(&h.env, "bad deposit"));
...
#[test]
fn test_failed_transaction_rejects_refund() {
    let h = setup();
    let id = register_default(&h);
    h.client
        .fail_transaction(&id, &String::from_str(&h.env, "bad deposit"));
...
#[test]
fn test_completed_transaction_rejects_refund() {
    let h = setup();
    let id = register_default(&h);
    h.client.confirm_transaction(&id);
    h.client.register_callback(&id);
